...
const MARKDOWN_PATCHED = Symbol.for("pi-copy-code.zero-code-block-padding");
...
function readEscape(text: string, index: number): string | undefined {
  ...
  if (text[index + 1] === "]") {
    ...
    return end >= 0 ? text.slice(index, end) : undefined;
  }

  return text.slice(index).match(/^\x1b\[[0-?]*[ -/]*[@-~]|^\x1b[@-Z\\-_]/)?.[0];
}

function removeLeadingRenderedSpaces(line: string, count: number): string {
  let index = 0;
  let removed = 0;
  let prefix = "";

  while (index < line.length && removed < count) {
    const escape = readEscape(line, index);
    if (escape) {
      prefix += escape;
      index += escape.length;
      continue;
  ...
  return prefix + line.slice(index) + " ".repeat(removed);
}

function patchMarkdownCodeBlockPadding(): void {
  const prototype = Markdown.prototype as any;
  if (prototype[MARKDOWN_PATCHED]) {
    return;
  }

  const originalRender = prototype.render;
  prototype.render = function patchedMarkdownRender(width: number): string[] {
    const lines: string[] = originalRender.call(this, width);
    const paddingX = Number(this.paddingX || 0);
    const markdown = String(this.text || "");

    if (!paddingX || !/^[ \t]*(`{3,}|~{3,})/m.test(markdown)) {
      return lines;
    }
    ...
    return lines.map((line) => {
      const visible = stripControls(line);
      const fence = visible.trimStart().match(/^(`{3,}|~{3,})/);
      let shouldFix = inCode;
      ...
      return shouldFix ? removeLeadingRenderedSpaces(line, paddingX) : line;
    });
  };

  prototype[MARKDOWN_PATCHED] = true;
}

export default function copyCodeExtension(pi: ExtensionAPI) {
  if (process.env.PI_COPY_CODE_ZERO_PADDING === "1") {
    patchMarkdownCodeBlockPadding();
  }
